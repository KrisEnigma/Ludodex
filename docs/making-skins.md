...
settings UI are ever needed — they all read the variables. Previews in the Settings 
swatch menu flow dynamically from the CSS classes, decoupling presentation from the registry.
...
- **Registry:** `src/skins/registry.ts` — defines the `SkinId` union, the `SKINS` array 
  holding configuration metadata (IDs, platform entitlement identifiers, and achievement targets), 
  and core interface stubs like `applySkin` / `normalizeSkinId` / `getCurrentSkinId`.
...
| `src/skins/registry.ts` | `SkinId` union member + `SKINS` metadata entry. |
...
**Oswald**, **Squada One**. If the skin reuses one of these, do nothing here.

Self-hosted (not on `@fontsource` — declared with `@font-face` at the top of
`skins.css`, the asset lives in `src/fonts/`): **DooM**, **AmazDooM***,
**Diablo**, **SuperMarioBrosWii**. Reusing one of these also needs no new dependency.
To add a *new* self-hosted font: drop the file in `src/fonts/`, add an `@font-face` block in
...
2. Add a `SKINS` metadata entry (template in §7.2). Previews in the selection menu
are applied via matching CSS classes, freeing the registry from layout data.
...
scale numbers and any low-contrast colors. Confirm the Settings swatch looks correct.
...
`restorePurchases` (native) re-checks entitlements. Previews are painted directly
by matching CSS variable scopes.

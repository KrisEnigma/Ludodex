...
**Served at:** `https://ludodex.krisenigma.com/.well-known/assetlinks.json`
...
```
curl -I https://ludodex.krisenigma.com/.well-known/assetlinks.json
```
Expect: `HTTP/2 200`, `content-type: application/json`.

Then verify Android can read it:
```
adb shell pm verify-app-links --re-verify app.ludodex.game
adb shell pm get-app-links app.ludodex.game
```

When the verification succeeds, tapping a `https://ludodex.krisenigma.com/123`
link from any other app (Messages, Slack, Gmail) opens Ludodex directly.
...
```json
{
  "applinks": {
    "details": [
      {
        "appIDs": ["TEAMID.app.ludodex.game"],
        "components": [
...
**Served at:** `https://ludodex.krisenigma.com/.well-known/apple-app-site-association`
...
Associated Domains capability and adding `applinks:ludodex.krisenigma.com`

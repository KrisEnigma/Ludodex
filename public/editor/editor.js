...
function loadPuzzleIntoEditor(puzzle, id) {
  ...
  Object.entries(d).forEach(([key, val]) => {
    if (key.startsWith('word')) {
      const wId = key;
      wordMap[wId] = { id: wId, display: val.display || '', path: val.path || '', color: WC[Object.keys(wordMap).length % WC.length] };
    }
  });
  S.words = Object.values(wordMap);
...
function toJSON(meta, words, letters) {
  ...
  words.forEach((w, i) => {
    ...
    data[`word${i + 1}`] = { display: w.display, path: w.path };
  });
  const filler = {};

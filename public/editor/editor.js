...
function renderMeta() {
  ...
  form.innerHTML = `
    <div class="form-field">
      <label class="form-label" for="m-id">Puzzle ID</label>
      <input class="form-input" id="m-id" type="text" value="${escapeHtml(m.id)}" placeholder="e.g. puzzle-042" oninput="S.meta.id=this.value;renderAll()" autocomplete="off">
    </div>
    <div class="form-field">
      <label class="form-label" for="m-cat">Category</label>
      <select class="form-input" id="m-cat" onchange="S.meta.category=this.value;renderAll()">
    ...
    <div class="form-field form-field--checkbox">
      <label class="checkbox-label">
        <input type="checkbox" ${m.premium?'checked':''} onchange="S.meta.premium=this.checked;renderAll()">
        <span>Premium puzzle</span>
      </label>
    </div>`;
}
...
function renderListItem(p, i) {
  ...
  <div class="list-item${isExpanded ? ' is-expanded' : ''}" draggable="true" data-idx="${i}" data-id="${p.id}">
    ...
    <div class="list-item-info">
      <div class="list-item-id">${escapeHtml(p.id)}</div>
      <div class="list-item-name">${escapeHtml(name) || '<span style="color:var(--color-text-faint)">No name</span>'}</div>
    </div>
    ...
    <div class="list-item-actions">
      <button class="btn btn-ghost btn-icon" onclick="listToggleExpand('${p.id}')" title="${isExpanded ? 'Collapse' : 'Edit metadata'}" aria-label="${isExpanded ? 'Collapse' : 'Edit metadata'}">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="transform:rotate(${isExpanded ? '180deg' : '0deg'});transition:transform 180ms ease">
          <polyline points="6 9 12 15 18 9"/>
        </svg>
      </button>
      <button class="btn btn-ghost btn-icon" onclick="listOpenFull('${p.id}')" title="Open in full editor" aria-label="Open in full editor">

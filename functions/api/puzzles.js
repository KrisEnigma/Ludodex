...
export async function onRequest({ request, env }) {
  ...
  if (request.method === 'PUT') {
    ...
    let parsed;
    try { parsed = JSON.parse(body); } catch { return err('Invalid JSON', 400); }
    if (!Array.isArray(parsed) || parsed.length === 0) {
      return err('Payload must be a non-empty JSON array', 400);
    }
    await env.PUZZLE_BUCKET.put(PUZZLES_KEY, body, {
      httpMetadata: { contentType: 'application/json' },
      customMetadata: { updatedAt: new Date().toISOString(), count: String(parsed.length) },
    });
